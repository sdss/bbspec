...
if opts.test:
    if opts.image is None:
        ...
        print 'TEST: Input image not set, using -i', opts.image
    if opts.psf is None:
        opts.psf = 'spBasisPSF-PIX-r1-00115982.fits'
        print 'TEST: Input psf not set, using -p', opts.psf
    if opts.output is None:
        opts.output = 'spSpec-r1-00115982.fits'
        print 'TEST: Output spectra not set, using -o', opts.output
    if opts.fluxbins is None:
        opts.fluxbins = "3000,3500,20"    
        print "TEST: Flux bin range not set, using -f", opts.fluxbins
    if opts.bundle is None:
        opts.bundle = "0"
        print "TEST: Bundle range not set, using -b", opts.bundle
#--- TEST ---
